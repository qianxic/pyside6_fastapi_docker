...
class MixedBCDDataset(torch.utils.data.Dataset):
    """
    混合多个二值变化检测数据集的类
    """
    def __init__(self, file_roots, splits, transform=None):
        """
        初始化混合数据集
        
        Args:
            file_roots: 列表，包含多个数据集的根目录路径
            splits: 列表，包含对应的分割名称（'train', 'val', 'test'）
            transform: 可选的数据转换
        """
        self.datasets = []
        
        # 创建每个数据集的实例
        for file_root, split in zip(file_roots, splits):
            try:
                dataset = BCDDataset(file_root=file_root, split=split, transform=transform)
                self.datasets.append(dataset)
                print(f"加载数据集: {file_root}, 分割: {split}, 样本数: {len(dataset)}")
            except Exception as e:
                print(f"警告：无法加载数据集 {file_root} 的 {split} 分割: {str(e)}")
        
        # 确保至少有一个有效数据集
        if not self.datasets:
            raise ValueError("没有可用的数据集被加载！")
